(ns waymo.main
  "Kotodama WASM entrypoint for the Waymo clean-room actor (L4) — Clojure port.
  ...
  facts (`waymo.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"waymo.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "waymo-compat" :tier tier :entities entities} 200])
